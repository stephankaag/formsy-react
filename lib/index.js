...
var Formsy = function (_React$Component) {
  ...
  function Formsy(props) {
    ...
    _this.runValidation = function (input, name) {
      var value = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : input.currentValue;
      ...
      var validationResults = (0, _utils.runRules)(value, currentValues, input.parsedValidations, _validationRules2.default);

      var requiredResults = (0, _utils.runRules)(value, currentValues, input.parsedRequiredValidations, _validationRules2.default);

      var isRequired = Object.keys(input.parsedRequiredValidations).length ? !!requiredResults.success.length : false;
    ...
    _this.isChanged = function () {
      return !(0, _utils.isSame)(_this.getPristineValues(), _this.getCurrentValues());
    };
      ...
      _this.setState(function (prevState) {
        return {
          inputs: mapInputState(prevState, function (input, name) {
            ...
            var validation = _this.runValidation(input, name);
    ...
    _this.resetValue = function (name) {
      _this.setState({
        inputs: _extends({}, _this.state.inputs, _defineProperty({}, name, _extends({}, _this.state.inputs[name], {
          currentValue: _this.state.inputs[name].pristineValue
        })))
      }, function () {
        _this.validate(name);
      });
    ...
    _this.setValidations = function (name, validations, required) {
      var otherValidations = (0, _utils.convertValidationsToObject)(validations) || {};

      var requiredValidations = required === true ? { isDefaultRequiredValue: true } : (0, _utils.convertValidationsToObject)(required);

      if (_this._isMounted) {
        _this.setState(function (prevState) {
          return {
            inputs: _extends({}, prevState, _defineProperty({}, name, _extends({}, prevState[name], {
        ...
        _this.preMountInputState[name].parsedValidations = otherValidations;
        _this.preMountInputState[name].parsedRequiredValidations = requiredValidations;
      }
    };
    ...
    _this.methodsToPassToChildren = {
      ...
      isValidValue: function isValidValue(name, value) {
        return _this.runValidation(_this.state.inputs[name], name, value).isValid;
      },
      setValue: _this.setValue,
      resetValue: _this.resetValue,
      setValidations: _this.setValidations
    };
    return _this;
  }
  ...
  _createClass(Formsy, [{
    ...
    value: function componentDidUpdate(prevProps, prevState) {
      ...
      var inputNames = Object.keys(this.state.inputs);

      if (inputNames.length != Object.keys(prevState.inputs).length || inputNames.some(function (name) {
        return !(0, _utils.isSame)(_this3.state.inputs[name].currentValue, prevState.inputs[name].currentValue);
      })) {
        this.validateForm();
      }
    }
    ...
    value: function render() {
      ...
      var _props = this.props,
          ...
          nonFormsyProps = _objectWithoutProperties(_props, ['getErrorMessage', 'getErrorMessages', 'getValue', 'hasValue', 'mapping', 'onChange', 'onInvalidSubmit', 'onInvalid', 'onReset', 'onSubmit', 'onValid', 'onValidSubmit', 'preventExternalInvalidation', 'resetValue', 'validationErrors']);
